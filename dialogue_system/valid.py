...
def dialogue_generation(malicious_model,model,tokenizer, args):
    ...
    for start_sentence in tqdm(sample(prompt_pool,args.query_number)):
        ...
        if args.job_name in ['BB','BBm','BBl']:
            ...
            #         chat_history_ids=tokenizer.encode(tokenizer.decode(new_user_input_ids[:,:][0])+ tokenizer.eos_token+generation_setnence, return_tensors='pt').to(args.device)
            #     else:
            #         conversation = chatbot(conversation)
            #         generation_setnence=conversation.generated_responses[-1]
            #         chat_history_ids=tokenizer.encode(start_sentence+ tokenizer.eos_token+generation_setnence, return_tensors='pt').to(args.device)
            #     # generated a response while limiting the total chat history to 1000 tokens    
            #     # chat_history_ids = model.generate(
            #     #     bot_input_ids, max_length=1024,
            #     #     pad_token_id=tokenizer.eos_token_id,
